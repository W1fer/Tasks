(function ($) {
	$(".contact-form").submit(function (event) {
		...
		let inpNameError = $(this).find('.contact-form__error_F_L_Name_P');
		let inpEmailError = $(this).find('.contact-form__error_email');
		let inpTelError = $(this).find('.contact-form__error_tel');
		let inpTextError = $(this).find('.contact-form__error_comment');
		let inpAddressError = $(this).find('.contact-form__error_address');
		...
		$.ajax({
			...
			success: function success(res) {
				...
				if (respond.address) {
					inpAddressErrorntError.text(respond.address);
				} else {
					inpAddressError.text('');
				}
